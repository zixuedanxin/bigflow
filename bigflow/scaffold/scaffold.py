...
from bigflow.scaffold.scaffold_templates import beam_workflow_template, beam_processing_template, \
    ...
    advanced_deployment_config_template, docker_template, beam_config_template
...
def create_dirs(config):
    ...
    main_templates = {'project_setup.py': project_setup_template, 'deployment_config.py': deployment_config_template, 'Dockerfile': docker_template}
    project_path = Path(config['project_name']).resolve()
    beam_workflow_path = Path(config['project_name'] + '/workflows')
